#!/usr/bin/env python3
"""
Sample subsets of ethernet and ipv4 tables from lo.json.
Creates n files with evenly-spaced table sizes from minimal to full size,
maintaining corresponding indices between ethernet and ipv4 tables.
"""
...
def separate_tables(data):
    ...
    ethernet_entries = []
    ipv4_entries = []
    ethernet_catchall = None
    ipv4_catchall = None
    other_entries = []
    
    for entry in data:
        table_name = entry.get('table', '')
        if table_name == 'ethernet':
            if is_wildcard_rule(entry):
                ethernet_catchall = entry
            else:
                ethernet_entries.append(entry)
        elif table_name == 'ipv4':
            if is_wildcard_rule(entry):
                ipv4_catchall = entry
            else:
    ...
    return ethernet_entries, ipv4_entries, ethernet_catchall, ipv4_catchall, other_entries


def create_sample(ethernet_entries, ipv4_entries, ethernet_catchall, ipv4_catchall, 
                  other_entries, sample_size):
    """
    Create a sample with the specified number of ethernet/ipv4 entries.
    Maintains the same indices in both tables.
    Always includes the catch-all rules at the end.
    """
    sampled_ethernet = ethernet_entries[:sample_size]
    sampled_ipv4 = ipv4_entries[:sample_size]
    
    # Add catch-all rules if they exist
    if ethernet_catchall:
        sampled_ethernet.append(ethernet_catchall)
    if ipv4_catchall:
        sampled_ipv4.append(ipv4_catchall)
    
    # Combine: other entries + sampled ethernet + sampled ipv4
    return other_entries + sampled_ethernet + sampled_ipv4
...
def generate_experiments_json(template_file, sample_sizes, base_name, output_dir):
    ...
    Args:
        ...
        base_name: Base name of the sampled files (e.g., "lo")
    ...
    for size in sample_sizes:
        for exp in template_experiments:
            ...
            # Pattern 1: lo_ad.json -> lo1_ad.json
            ...
            # Pattern 2: ad_lo.json -> ad_lo1.json
    ...
    # For path like ../programs/retargeting/data/experiment5, we want ../programs/retargeting
    output_dir_path = Path(output_dir_str)
    # Go up two levels: experiment5 -> data -> retargeting
...
def generate_samples(input_file, num_samples, output_dir=None, experiments_template=None):
    ...
    ethernet_entries, ipv4_entries, ethernet_catchall, ipv4_catchall, other_entries = separate_tables(data)
    
    # Verify both tables have the same size
    if len(ethernet_entries) != len(ipv4_entries):
        print(f"Warning: ethernet table has {len(ethernet_entries)} entries, "
              f"ipv4 table has {len(ipv4_entries)} entries")
    
    max_size = min(len(ethernet_entries), len(ipv4_entries))
    print(f"Total ethernet entries: {len(ethernet_entries)}")
    print(f"Total ipv4 entries: {len(ipv4_entries)}")
    if ethernet_catchall:
        print(f"Ethernet catch-all rule found: {ethernet_catchall.get('matches', {})}")
    if ipv4_catchall:
        print(f"IPv4 catch-all rule found: {ipv4_catchall.get('matches', {})}")
    ...
    for i, size in enumerate(sample_sizes):
        sample_data = create_sample(ethernet_entries, ipv4_entries, 
                                   ethernet_catchall, ipv4_catchall,
                                   other_entries, size)
        ...
        print(f"  Table size: {size} entries each (ethernet & ipv4) + catch-all rules")
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python sample_lo_tables.py <input_file> <num_samples> [output_dir] [experiments_template]")
        print("Example: python sample_lo_tables.py programs/retargeting/data/lo.json 5")
        print("Example: python sample_lo_tables.py programs/retargeting/data/lo.json 5 output_samples/")
        print("Example: python sample_lo_tables.py programs/retargeting/data/lo.json 5 programs/retargeting/data programs/retargeting/experiments.json")
        sys.exit(1)
