...
let test_filter () =
  ...
  in
  let result_exact = BaseInterpreter.eval transform_exact config in
  ...
  in
  let result_multi = BaseInterpreter.eval transform_multi config in
  ...
  in
  let result_proto = BaseInterpreter.eval transform_proto config in
  ...
  in
  let result_ternary = BaseInterpreter.eval transform_ternary config in
  ...
  in
  let result_empty = BaseInterpreter.eval transform_empty config in
  ...
  in
  let result_nonexistent = BaseInterpreter.eval transform_nonexistent config in
  ...
  in
  let result_composed = BaseInterpreter.eval outer_transform config in
  ...
  let transform_conflict =
    BaseLogic.TransformExpr.Filter
      ( BaseLogic.TransformExpr.Filter
        ...
        conflicting_filter )
  in
  let result_conflict = BaseInterpreter.eval transform_conflict config in
...
let test_transform_table_symbol () =
  ...
  let result = BaseInterpreter.eval transform_expr config in
...
let test_bitvec_expressions () =
  ...
  in
  let result = BaseInterpreter.eval emit_expr config in
  let result_row = List.hd_exn result in
  ...
  in
  let and_result = BaseInterpreter.eval and_expr config in
...
let test_transform_to_clause_encoding () =
  ...
  in
  let transform_result = BaseInterpreter.eval transform_filter config in
...
let test_filter_action_constraint () =
  ...
  let result = BaseInterpreter.eval transform_filter config in
  let result_row = List.hd_exn result in
  let action_data = Action.get_data result_row.action in
