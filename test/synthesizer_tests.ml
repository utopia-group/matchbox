...
let test_inverse () =
  ...
  let phi (cand : t) =
    match cand.definition with
    | Clause.Inverse f' ->
      Symbol.(f' = target) && Symbol.(cand.defined = target)
    | _ -> false
  in
  let progs = BaseSynthesizer.synth phi in
  check bool "Found at least one Inverse(F3)" true (not (List.is_empty progs));
  let inv_prog =
    List.find_exn progs ~f:(fun p ->
        match p.definition with Clause.Inverse _ -> true | _ -> false)
  in
  match inv_prog with
  | {defined; definition = Clause.Inverse f'} ->
    check string "Defined is F3" "F3" defined.name;
    check string "Inverse is F3" "F3" f'.name
  | _ -> fail "Expected Clause.Inverse F3"

(* Test that synthesizer can find both Id and Inverse for different symbols *)
let test_multiple_clause_kinds () =
  ...
  let phi (cand : t) =
    ...
    | Clause.Id f' when String.(f'.name = "F0" && cand.defined.name = "F0") ->
      true
    | Clause.Inverse f' when String.(f'.name = "F1" && cand.defined.name = "F1")
      ->
      true
    | _ -> false
  in
  ...
  let has_inv =
    List.exists progs ~f:(fun p ->
        match p.definition with
        | Clause.Inverse f' when String.(f'.name = "F1") -> true
        | _ -> false)
  in

  check bool "Has identity clause" true has_id;
  check bool "Has inverse clause" true has_inv;
...
let test_synthesis_variety () =
  ...
  let phi (cand : t) =
    ...
    | Clause.Inverse f' -> Symbol.(f' = target)
  ...
  check bool "Has inverse" true (Set.mem clause_types "Inverse")
